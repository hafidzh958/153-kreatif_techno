...
@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.portfolio.update', $portfolio) }}" method="POST" class="space-y-4" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                class="w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $portfolio->description ?? '') }}</textarea>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        <div>
            <label for="image" class="block text-sm font-medium text-gray-700 mb-1">Gambar (kosongkan jika tidak ubah)</label>
